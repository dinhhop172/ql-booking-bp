...
if(isset($_POST['createUser'])){
    ...
    if(!$error){
        if(create_new_user($data, $conn)){
            $_SESSION['success'] = '<script>alert("Add user thành công")</script>';
            header('location: ../../../views/admin/users/index.php');
        }else{
            $_SESSION['error'] = '<script>alert("Add user thất bại")</script>';
            header('location: ../../../views/admin/users/index.php');
        }
    }else{
        $_SESSION['error'] = $error;
        $_SESSION['data'] = $data;
        return header('Location: ../../views/admin/users/create.php');
    }
}else{
    die('error');
}
